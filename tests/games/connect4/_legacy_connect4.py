"""
Legacy Connect4 implementation preserved for comparison testing.

This is a copy of the original numpy-based implementation before
bitboard optimization. Used to verify the new implementation produces
identical results.
"""
...
class LegacyConnect4State(State):
    """
...
class LegacyConnect4(Game):
    action_size: int = 7

    def reset(self) -> LegacyConnect4State:
        board: Int[np.ndarray, "6 7"] = np.zeros((6, 7), dtype=np.int8)
        return LegacyConnect4State(board=board)

    def to_play(self, s: LegacyConnect4State) -> int:
        # In this representation, the player-to-move is always the +1 player.
        return +1

    def legal_actions(self, s: LegacyConnect4State) -> Bool[np.ndarray, "7"]:
        # A move is legal if the column is not full (top cell is empty).
        legal = (s.board[0, :] == 0)
        return legal.astype(np.bool_, copy=False)

    def next_state(self, s: LegacyConnect4State, a: int) -> LegacyConnect4State:
        ...
        return LegacyConnect4State(board=board2)

    def terminal_value(self, s: LegacyConnect4State) -> tuple[bool, float]:
        """
    ...
    def encode(self, s: LegacyConnect4State) -> Float[np.ndarray, "6 7"]:
        """
    ...
    def key(self, s: LegacyConnect4State) -> bytes:
